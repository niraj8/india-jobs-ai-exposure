...
def main():
    ...
    # Merge: plfs_stats is the canonical list of 127 groups
    data = []
    for p in plfs:
        g = p["nco_group"]
    ...
    total = sum(d["workers"] for d in data)
    with_pay = sum(1 for d in data if d["median_pay"] is not None)
    print(f"Wrote {len(data)} groups to site/data.json")
    print(f"Groups with pay data: {with_pay}")
    print(f"Total estimated workers: {total:,}")
